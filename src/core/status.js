...
export function workspaceStatus(store) {
  ...
  const activeFacts = facts.filter((item) => item.truth_status === "active");
  const revokedFacts = facts.filter((item) => item.truth_status === "revoked");
  const states = {};
  ...
  return {
    ...
    facts: { total: facts.length, active: activeFacts.length, revoked: revokedFacts.length, revocation_receipts: revocations.length },
...
export function graphLog(store) {
  ...
  const visit = (node, prefix = "", connector = "") => {
    ...
    const state = node.layer === "truth"
      ? node.truth_status
      : [node.state, node.promoted_truth_status ? `promoted truth ${node.promoted_truth_status}` : null].filter(Boolean).join("; ");
    lines.push(`${prefix}${connector}${id} [${node.layer}/${node.kind}] ${node.title ?? ""}${state ? ` (${state})` : ""}`);
